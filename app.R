...
pivot_consumer <- Pivot(
  PivotItem(headerText = "Изменение цен - г/г", 
            info_card(text = tags$span('На графике жирной линией обозначено, как изменилась цена выбранного товара по сравнению с аналогичным периодом предыдущего года. Общее', tags$b(' изменение цены раскладывается на влияние отдельных факторов '), 'релевантных для данного рынка.',
  ...
  PivotItem(headerText = "Изменение цен - к предыдущему периоду", 
            info_card(text = tags$span('На графике жирной линией обозначено, как изменилась цена выбранного товара по сравнению с предыдущем месяцем/неделей. Общее', tags$b(' изменение цены раскладывается на влияние отдельных факторов '), 'релевантных для данного рынка.',
                                       tags$p(), 'Под инфляцией спроса понимается изменение динамики спроса на товар, а также влияние общеинфляционных тенденций в экономике на изменение его цены.',
                                       tags$p(), 'Затемненная область соответствует прогнозным значениям, полученным на основе построенной статистической модели. 
                                       Для прогнозов также приводятся доверительные интервалы, обозначающие диапазон, в котором наиболее вероятно будет находиться значение инфляции для выбранного продукта.',
                                       tags$p(), 'Подробное описание используемых в анализе факторов доступно в разделе "Пояснения к факторам"'),
  ...
  PivotItem(headerText = "Структура розничной цены", 
            info_card(text = tags$span("На графике представлена динамика средней цены выбранного товара", tags$b(" в разбивке по основным статьям структуры розничной цены "), "(согласно ежегодным данным Росстата)."),
...
pivot_prodprices <- Pivot(
  ...
  PivotItem(headerText = 'Внешнеторговые цены',
            info_card(text = tags$span("На графике изображены", tags$b(" средние цены импорта и экспорта "), "товаров, входящих в рассматриваемые товарные группы. Они получены как частное общей стоимости проданной продукции и ее количества в натуральном выражении.",
                                       tags$p(), "По умолчанию результат конвертируется в рубли по среднемесячному валютному курсу. Для отображения исходных данных в долларах США необходимо", tags$b(" воспользоваться переключателем справа.")),
...
pivot_market <- Pivot(overflowBehavior="menu",
  PivotItem(headerText = "Динамика совокупного предложения", 
            info_card(text = tags$span("На графике представлена динамика", tags$b(" расчетного объема совокупного предложения "), "на рынке. Предложение складывается из внутреннего производства, изменения запасов и сальдо чистого экспорта.",
                                       tags$p(), "Все расчёты производится в натуральном выражении. По умолчанию используются сезонно-сглаженные данные. Для отображения исходных значений необходимо", tags$b(" воспользоваться переключателем справа.")),
  ...
  PivotItem(headerText = "Географическая структура производства",
            info_card(text = tags$span("Карта отражает", tags$b(" географическое распределение внутреннего производства товара "),"в России. Выбор анализируемого периода производится с помощью календаря справа от карты. По умолчанию данные представлены на уровне федеральных округов. 
  ...
  PivotItem(headerText = "Региональная динамика производства",
            info_card(text = tags$span("Карта отражает", tags$b(" динамику внутреннего производства товара"), " в России. Выбор анализируемого периода производится с помощью календаря справа от карты. По умолчанию данные представлены на уровне федеральных округов. 
  ...
  PivotItem(headerText = "Внешняя торговля",
            info_card(text = tags$span("На графиках представлены показатели, характеризующие состояние внешней торговли рассматриваемым товаром.",
                                       tags$p(),
                                       "Для выбранного в календаре месяца отображаются ", tags$b("три крупнейших направления экспорта и импорта."),
                                       tags$p(),
                                       "Показатель", tags$b(" географической концентрации "), "рассчитан как индекс Херфиндаля-Хиршмана на основе долей отдельных торговых партнеров в экспорте и импорте.
  ...
  PivotItem(headerText = 'Розничные продажи',
            info_card(text = tags$span("На графике изображен совокупный", tags$b(" объем розничных продаж "), "в выбранной товарной группе."),
                      toggle_id = "retail_sa",
                      toggleOn = "Сезонное сглаживание включено",
                      toggleOff = "Сезонное сглаживание отключено",
                      display_download=FALSE),
  ...
  PivotItem(headerText = 'Уровень наценки',
            info_card(text = tags$span("На графике изображены средние", tags$b(" уровни фактически сложившейся торговой наценки"), " в сфере оптовой и розничной торговли товарами выбранной группы."),
                      toggle_id = NULL,
                      display_download=FALSE),
...
cntf_page <- makePage(
  ...
  contents = div(
    info_card(text = tags$span(HTML("На информационной панели представлены результаты <b>оценки доли нелегального оборота продукции на квартальном уровне</b>. <p> Расчет производится с помощью
                                балансового метода, предполагающего сопоставление <b>легального предложения</b> (производство + экспорт - импорт) и <b>объема розничных продаж</b>")),
              toggle_id = NULL,
              download_id = NULL,
              display_download=FALSE),
...
balance_page <- makePage(
  ...
  contents = div(
    info_card(text = tags$span(HTML('На графике представлен <b>баланс фактических затрат и выгод</b>, реализовавшихся в отрасли после введения
                                    обязательной цифровой маркировки. Расчет производится на основе данных статистики Росстата. <p>Для <b>объемов производства,
                                    занятости, загрузки производственных мощностей и налоговых поступлений</b> из точки 
                                    "до введения ОЦМ" строится трендовый прогноз развития отрасли для сценария, в котором маркировка не вводится.
                                    Эффект от введения ОЦМ определяется как отклонение фактической отраслевой динамики от прогнозных значений.<br>
                                    <p>Под <b>издержками маркировки</b> понимается совокупный объем затрат, отраженных в потребительских ценах
                                    и переложенных на потребителя. Для расчета используются оценки, полученные на основе моделей из раздела "Потребительские цены"')),
              toggle_id = NULL,
              download_id = NULL,
              display_download=FALSE),
    tags$div(
